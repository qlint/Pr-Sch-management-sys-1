...
class AndroidApiController extends CController
{
			...
			if(isset($post['tag']) && $post['tag'] != ''){
				...
				elseif($tag == 'exams'){ //exams.......				
					...
					if(key($role)!=NULL and key($role) == 'teacher'){
						...
						//My Class
						$criteria				= new CDbCriteria();								
						$criteria->condition	= 'employee_id=:employee_id AND is_active=:is_active AND is_deleted=:is_deleted';
						$criteria->params		= array(':employee_id'=>$employee->id, ':is_active'=>1, ':is_deleted'=>0);
						$criteria->order		= 'name ASC';
						...
						$criteria->condition	= '`t1`.`employee_id`=:employee_id AND `t`.`is_active`=:is_active AND `t`.`is_deleted`=:is_deleted';
						$criteria->params		= array(':employee_id'=>$employee->id, ':is_active'=>1, ':is_deleted'=>0);
				...
				elseif($tag == 'add_attendence'){ //Add Attendance
					if((isset($post['uid']) and $post['uid'] != NULL)){
						$uid			= $post['uid'];
						$settings		= UserSettings::model()->findByAttributes(array('user_id'=>1));
						$students_array	= $_POST['students']; 
						$data_string	= $_POST['data'];
						$batch_data		= json_decode($data_string);
						$student_data	= $batch_data[0]->students;						
						$header			= $batch_data[0]->header;                                                                                        
						$batch_id 		= $header->batch_id;
						$date			= date('Y-m-d', strtotime($header->timestamp));									   
						foreach($student_data as $data){							
							...
							$leave_type		= $data->status->late_type;							
							$leave_model	= StudentAttentance::model()->findByAttributes(array('student_id'=>$student_id,'batch_id'=>$batch_id,'date'=>$date));
							if($leave_model){                
								if($data->status->present == 1){
									$leave_model->delete();
								}
								else{
									$leave_model->reason		= $reason;
									$leave_model->leave_type_id	= $leave_type;
									$leave_model->save();
								}
							}
							else{							
								...
								$model->reason= $reason;
								$model->date= $date;
								$model->leave_type_id= $leave_type;
								if($model->save()){
				...
				elseif($tag == 'change_profile'){
					...
					if($type == 1){ //In case of student
						...
						if($model->uid != 0){
							$achievements	= Achievements::model()->findAllByAttributes(array('user_id'=>$model->uid));
						}
					}
	...
	protected function getGrade($batch_id, $mark){
		...
		if($grading_levels == NULL){
			$grading_levels	= GradingLevels::model()->findAllByAttributes(array('batch_id'=>NULL));	
		}
