...
class ConfigurationsController extends RController
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['submit']))
		{
			...
			if(isset($_POST['dateformat']) /*&& (isset($_POST['timeformat']))*/ && isset($_POST['timezone'])&& isset($_POST['language']))
			{
				...
				$settings->save();
                                Yii::app()->translate->setLanguage($settings->language);
			}
        ...
        public function actionAuthentication()
	{
		...
		$model->authentication = $var1->config_value;
		if(isset($_POST['Configurations']))
		{
			if(isset($_POST['Configurations']['authentication']))
				{                            
					if($_POST['Configurations']['authentication']=='1')
					{
						$var1->config_value = 1;
						$var1->save();
					}
				...
				Yii::app()->user->setFlash('successMessage', Yii::t('app',"Action performed successfully"));
				$this->redirect(array('configurations/authentication'));
		}
		$this->render('authentication', array('model'=>$model,));
	}
	public function actionExamformat()
	{
		$model = new Configurations;
		
		$var1 = Configurations::model()->findByAttributes(array('id'=>41));
		
		
		if(isset($_POST['Configurations']))
		{
			
			if(isset($_POST['Configurations']['exam_format']))
				{ 
					if($_POST['Configurations']['exam_format']=='1')
					{
						$var1->config_value = 1;
						$var1->save();
					}
					elseif($_POST['Configurations']['exam_format']=='2')
					{
						$var1->config_value = 2;
						$var1->save();
					}
					elseif($_POST['Configurations']['exam_format']=='-1')
					{
						$var1->config_value = -1;
						$var1->save();
					}
				...
				/*
				Yii::app()->user->setFlash('successMessage', Yii::t('app',"Action performed successfully"));
				$this->redirect(array('configurations/examformat'));*/
		}
		$model->exam_format = $var1->config_value;
		//Yii::app()->user->setFlash('successMessage', Yii::t('app',"Action performed successfully"));
		$this->render('examformat',array('model'=>$model,));
	}
