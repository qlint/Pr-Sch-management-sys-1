...
<div class="top-hed-btn-right">
        <ul>
            ...
            <li><span>
            <?php 
				echo CHtml::link(Yii::t('app','View Subject List'), array('/teachersportal/exams/classexamresult','bid'=>$batch_id,'exam_group_id'=>$exam_group_id),array('id'=>'add_exam-groups','class'=>'btn btn-primary')); 
            ...
            <li><span>
            <?php 
				echo CHtml::link(Yii::t('app','View Exam List'), array('/teachersportal/exams/classexams','bid'=>$batch_id),array('id'=>'add_exam-groups','class'=>'btn btn-primary')); 
			...
			?>
            <li><span>
        	<?php echo CHtml::link(Yii::t('app','Change').' '.Yii::app()->getModule('students')->fieldLabel("Students", "batch_id"), array('/teachersportal/exams/classexam'),array('id'=>'add_exam-groups','class'=>'btn btn-primary')); ?>
        	</span></li>
        </ul>
        </div>
	...
	if((Yii::app()->controller->action->id=='cbscclassexamscore' and ($is_classteacher!=NULL)) or (Yii::app()->controller->action->id=='allexam' and $is_teaching_subject >0))
	...
	<!-- Start Enter Exam Scores -->
        ...
        if(isset($batch_id))
		{
			...
			if($students!=NULL)
    		{
				if(Yii::app()->controller->action->id=='cbscclassexamscore'){
					$actionUrl = CController::createUrl('/teachersportal/examScores/Cbscaddscores',array("bid"=>$batch_id,"exam_group_id"=>$exam_group_id,"exam_id"=>$exam_id));
				}				
...
<table class="table table-bordered mb30">
					...
					$k=0;
	  				foreach($students as $student){ 
						$checksub = CbscExamScores::model()->findByAttributes(array('exam_id'=>$exam_id,'student_id'=>$student->id));
						 $exm = CbscExams::model()->findByAttributes(array('id'=>$exam_id));
					...
					?>
                    			<tr>
									...
									<?php 
									if($subject_cps !=NULL){
									echo $form->textField($model,'marks['.$k.']',array('size'=>7,'maxlength'=>7,'class'=>'form-control','style'=>'width:200px','readonly'=>true));
									}else{
										echo $form->textField($model,'marks['.$k.']',array('size'=>7,'maxlength'=>7,'class'=>'form-control','style'=>'width:200px'));
									}?>
                                    </td>
                                    <td><?php echo $form->textField($model,'remarks['.$k.']',array('size'=>60,'maxlength'=>255,'style'=>'width:200px','class'=>'form-control')); ?></td>
								</tr>
					...
					if($i==1)
					{
						 ...
						 $allscores = CbscExamScores::model()->findAllByAttributes(array('exam_id'=>$exam_id));
						 $sum=0;
	...
	?>
    <?php
    $checkscores = CbscExamScores::model()->findByAttributes(array('exam_id'=>$exam_id));
	if($checkscores!=NULL)
	{?>
    <div>
        <div>
...
<div class="top-hed-btn-right">
                    <ul>
                        ...
                        <?php 
						//if(Yii::app()->controller->action->id=='classexamscore'){
							echo CHtml::link(Yii::t('app','Clear All Scores'), "#", array('submit'=>array('/teachersportal/examScores/cbscdeleteall',"id"=>$_REQUEST['bid'],"exam_group_id"=>$_REQUEST['exam_group_id'],"exam_id"=>$_REQUEST['exam_id']), 'class'=>'btn btn-primary','confirm'=>Yii::t('app','Are You Sure? All Scores will be deleted'), 'csrf'=>true));
						//}						
						?>
                        </li>
			...
			?>
            <!-- Start Score Table -->
            <?php $model=new CbscExamScores('search');
				...
				$new_array[]= array(
                 ...
                 elseif($score_flag==1){ // If $score_flag==1, score table with edit option will be displayed
				 	//if(Yii::app()->controller->action->id=='classexamscore'){
							$updateUrl = 'Yii::app()->createUrl("/teachersportal/examScores/cbscupdate", array("id"=>$data->id,"bid"=>'.$batch_id.',"exam_group_id"=>'.$exam_group_id.',"exam_id"=>'.$exam_id.'))';
							$delUrl = 'Yii::app()->createUrl("/teachersportal/examScores/cbscdelete", array("id"=>$data->id,"bid"=>'.$batch_id.',"exam_group_id"=>'.$exam_group_id.',"exam_id"=>'.$exam_id.'))';
						//}						
...
$("form#exam-scores-form").submit(function(e) {
	...
	else
	{
		...
		$.ajax({
			url:'<?php echo Yii::app()->createUrl("/teachersportal/examScores/Cbscaddscores", array("id"=>$_REQUEST['id'], "examid"=>$_REQUEST['examid']));?>',
			type:'POST',
			data:data,
			dataType:"json",
