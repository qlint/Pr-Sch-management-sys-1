...
<div class="contentpanel">    
    ...
    <div class="opnsl_actn_box">
        ...
        <div class="opnsl_actn_box2"><?php echo CHtml::link('<span>'.Yii::t('app','My Class').'</span>',array('/teachersportal/exams/classexam'),array('class'=>'addbttn last'));?></div>
	</div>
</div>
<div>
<?php
	...
	$batch=Batches::model()->findByAttributes(array('id'=>$batch_id));
        ...
        <div class="opnsl_headerBox">
            ...
            <div class="opnsl_actn_box">
                ...
                <div class="opnsl_actn_box1">
                	<?php echo CHtml::link(Yii::t('app','Change').' '.Yii::app()->getModule('students')->fieldLabel("Students", "batch_id"), array('/teachersportal/exams/index'),array('id'=>'add_exam-groups','class'=>'addbttn')); ?>
                </div>
            </div>
        </div>
	...
	<!-- Start Enter Exam Scores -->
        ...
        if(isset($batch_id))
		{
			$batchstudent =BatchStudents::model()->findAll("batch_id=:x and status=:y and result_status=:z", array(':x'=>$batch_id,':y'=>1,':z'=>0));
			$students = Students::model()->findAll("id=:x and is_active=:y and is_deleted=:z", array(':x'=>$batchstudents->student_id,':y'=>1,':z'=>0));
...
<table class="table table-bordered mb30">
					...
					$k=0;
	  				...
	  				foreach($batchstudent as $batchstudents){
						$student = Students::model()->findByAttributes(array('id'=>$batchstudents->student_id)); 
    ...
    <div class="opnsl_headerBox">
        <div class="opnsl_actn_box"> </div>
        <div class="opnsl_actn_box">
            <?php if($i!=1) echo CHtml::submitButton($model->isNewRecord ? Yii::t('app','Create') : Yii::t('app','Save'),array('class'=>'addbttn last')); ?>
        </div>
    </div>
                <?php $this->endWidget(); ?>
    	<?php
