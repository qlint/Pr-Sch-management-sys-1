...
?>
    ...
    <div align="center" style="display:block; text-align:center;">
		<h3><?php echo Yii::t('app','Weekly Subject Wise Attendance');?> - <?php echo $student->first_name.' '.$student->middle_name.' '.$student->last_name; 
			if($sem_enabled==1 and $batch->semester_id!=NULL){ ?>
			 	(<?php echo $course->course_name.' , '.$batch->name.' , '.$semester->name; ?>)
		<?php }
			  else{ ?>
			 	(<?php echo $course->course_name.' , '.$batch->name; ?>)
		<?php } ?>
		</h3>
	</div>
										  ...
										  if($set == NULL){
											...
											{	
												...
												$elective				=	Electives::model()->findByAttributes(array('batch_id'=>$batch->id, 'id'=>$set->subject_id)); 
													$student_elective	=	StudentElectives::model()->findByAttributes(array('student_id'=>$_REQUEST['student_id'], 'batch_id'=>$batch->id, 'elective_group_id'=>$elective->elective_group_id));
													...
													if($is_holiday == NULL){
														if($subjectwise == NULL){
															if($batch->start_date <= $week_start and $week_start <= $batch->end_date ){
																if($student->admission_date <= $week_start and $week_start <= date("Y-m-d")){
