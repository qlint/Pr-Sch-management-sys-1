...
?>
    ...
    <div align="center" style="display:block; text-align:center;">
		<h3><?php echo Yii::t('app','Daily Subject Wise Attendance');
 ...
 <div align="center" style="display:block; text-align:center;"><h5><?php echo date($displayformat, strtotime($date));?></h5></div>
  <?php 
    ...
    if(isset($timings) and $timings!=NULL){
		if($date > date('Y-m-d')){
			...
			if($is_holiday){
				echo '<i>'.Yii::t("app", "Selected day is a Holiday").'</i>';
			}
			else{
			...
			if($weekday==NULL){
				...
				<table  align="left" width="100%" id="table" cellspacing="0" cellpadding="0" class="timetable" >
                    ...
                    <?php if(Configurations::model()->rollnoSettingsMode() != 2){ ?>
                        	<td>
								<h3><?php if($batch_student!=NULL and $batch_student->roll_no!=0){
                    ...
                    <?php } ?>  
                            ...
                            <?php
								...
								if($set->is_elective == 2){ 
									$student_elective	=	StudentElectives::model()->findByAttributes(array('student_id'=>$student->id, 'batch_id'=>$batch->id, 'elective_id'=>$set->subject_id)); 
