						...
						foreach($timetable_entries as $timetable_entrie){
							if(!in_array($timetable_entrie->batch_id,$batches_ids)){
								//check curent batch  
										...
										$criteria1->condition = "`id`= '$b_id' AND `academic_yr_id`= '$accademic->id' AND`is_active`=1 AND `is_deleted`=0";						
                        	...
                        	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-bordered mb30">
                            	<thead>
                                	...
                                	{										
										...
										echo '<td>'.CHtml::link($course_name->course_name, array('/teachersportal/default/employeetimetable','id'=>$batch->id)).'</td>';
										echo '<td>'.CHtml::link($batch->name, array('/teachersportal/default/employeetimetable','id'=>$batch->id)).'</td>';
										...
										if($sem_enabled==1){
											if($sem_enabled_course == 1 and $batch->semester_id!=NULL){
                                                ...
                                                <?php
                                                    ...
                                                    else
                                                    {
														...
														if($time_emp!=NULL)
														{
															...
															{
																echo '<b>'.ucfirst($elec_group->name).'</b><br>';
																echo '<b>'.ucfirst($elec_sub->name).'</b><br>';
															if($set->is_elective==0){
...
<script>
 $('#acc_id').change(function(ev){
	var acc_id	= $(this).val();
	if(acc_id != ''){
		window.location= 'index.php?r=teachersportal/default/employeetimetable&acc_id='+acc_id;
	}
	else{
		window.location= 'index.php?r=teachersportal/default/employeetimetable';
	}
});
 </script>
