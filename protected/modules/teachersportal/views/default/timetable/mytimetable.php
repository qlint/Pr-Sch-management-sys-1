...
<div class="panel-heading">
            ...
            <br />
             ...
             <?php
				...
				if($timing!=NULL)
				{
					echo CHtml::link(Yii::t('app','Generate PDF'), array('Default/classPdf','id'=>$_REQUEST['id']),array('class'=>'btn btn-danger pull-right','target'=>'_blank'));
				} 
			 ?>
              <?php 
			   ...
			   if($_REQUEST['id']!=NULL){
					...
					$criteria1->condition = "`academic_yr_id`= '$accademic->id' AND`is_active`=1 AND `is_deleted`=0 AND employee_id=:emp_id";	
					$criteria1->params=array(':emp_id'=>$employee->id);					
					$batch		 =	Batches::model()->findAll($criteria1);
					$timetable_ids = array();
					if($batch){
						foreach($batch as $batchs){
							if(!in_array($batchs->id,$timetable_ids)){
								//check curent batch  
										//$today	=	date('Y-m-d');
										$b_id	=	$batchs->id;
										...
										$criteria->condition='batch_id=:emp_id';
										$criteria->params=array(':emp_id'=>$b_id);
										$timetable_entries = TimetableEntries::model()->findAll($criteria);
										if($timetable_entries!=NULL)							
											$timetable_ids[] = $batchs->id;
							}
						}
					}
			...
			//Check whether the teacher have any substitution in any batch		
							...
							if($is_in_timetable){
								if(in_array($is_any_substitute->date_leave,$date_between)){																						
									if(!in_array($is_any_substitute->batch,$batches_ids)){	
										...
										$criteria2->condition = "`start_date` <= '$today' AND `end_date` >= '$today' AND `id`= '$b_id' AND `academic_yr_id`= '$accademic->id' AND `is_active`=1 AND `is_deleted`=0";						
					...
					if(count($timetable_ids) > 1){ // List of batches is needed
						$list_flag = 2;	
					}
					elseif(count($timetable_ids) <= 0){ // If not teaching in any batch
						$list_flag = 0;
					}
					else{ // If only one batch is found
						$list_flag = 1;
						$_REQUEST['id'] = $timetable_ids[0];	
                        	...
                        	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-bordered mb30">
                            	<thead>
									...
									//var_dump($batches_ids);exit;
                          			for($i = 0; $i <count($timetable_ids); $i++)
                                	{										
										$batch		 =	Batches::model()->findByAttributes(array('id'=>$timetable_ids[$i],'is_active'=>1,'is_deleted'=>0));
...
<script>
 $('#acc_id').change(function(ev){
	var acc_id	= $(this).val();
	if(acc_id != ''){
		window.location= 'index.php?r=teachersportal/default/employeeClassTimetable&acc_id='+acc_id;
	}
	else{
		window.location= 'index.php?r=teachersportal/default/employeeClassTimetable';
	}
});
 </script>
