                        ...
                        { 
                            ...
                            ?>
                                <tr class=<?php echo $cls;?>>
                                    ...
                                    <td><?php 
                                    //$exists     =   OnlineExamStudentAnswers::model()->exists('exam_id = :exam_id AND student_id=:student_id',array('exam_id'=>$exam_id,'student_id'=>$data->id)); (old code)
									//$exists     =   OnlineExamStudents::model()->findByAttributes(array('exam_id'=>$exam_id,'student_id'=>$data->id, 'status'=>1));
									$criteria				= new CDbCriteria();
									$criteria->join			= 'JOIN online_exam_students t1 ON t1.student_id = t.student_id JOIN online_exam_questions t2 ON t2.id = t.question_id';
									$criteria->condition	= 't.student_id=:student_id AND t.exam_id=:exam_id AND t.is_verified=:is_verified';
									$criteria->params		= array(':student_id'=>$data->id, ':exam_id'=>$exam_id, ':is_verified'=>0);
									$evaluates				= OnlineExamStudentAnswers::model()->findAll($criteria);
                                    if(isset($_REQUEST['id']) && $_REQUEST['id']!=NULL && $evaluates!=NULL)
                                    {
