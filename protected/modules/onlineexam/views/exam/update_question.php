...
<div class="pageheader">
    <h2><i class="fa fa-pencil"></i><?php echo  Yii::t('app','Exams') .'<span>'.Yii::t('app','Online Exams here').'</span>'?></h2>
...
<div class="contentpanel">
    <div class="panel-heading" style="position:relative;">
        <div class="clear"></div>
        <h3 class="panel-title"><?php echo Yii::t('app','Update Question'); ?> </h3>
...
<div class="opnsl_headerBox">
            ...
            <div class="opnsl_actn_box1">
                <?php
                echo CHtml::link(Yii::t('app','Exams'),array('/onlineexam/exam/index','bid'=>$_REQUEST['bid']),array('class'=>'btn btn-primary'));
            ?></div>
            </div>
        ...
        $exam_model = OnlineExams::model()->findByPk($model->exam_id);
        if(isset($exam_model))
        {
            $multi_choice_limit=    $exam_model->choice_limit;
        }
        $this->renderPartial('update_question_form', array('model'=>$model));?>   
    </div>
</div>
...
$('#save_btn,#save_btns').click(function() {
...
function save_new_question(submit_type)
{
    ...
    //var question_answer =   $('#input[type="radio"][name="OnlineExamQuestions[choice_answer]"]:checked').val(); 
    //var choice_answer_id=   $('#RadioButtonAnswer input[type="radio"][name="OnlineExamQuestions[choice_answer_id]"]:checked').val(); 
    ...
    $.ajax({
		type:'POST',
		url:'<?php echo Yii::app()->createUrl('/onlineexam/exam/updateQuestion');?>',
...
function re_arrange_questions_order(){
	...
	$('#choices .choice-data').each(function(index, element) 
        {            
            ...
            $(this).closest("div").find(".main").attr('data',start);
		start++;
        });
}
