...
?>
                ...
                <div class="exam-table exam-table-line">
                    <table border="0" cellpadding="0" cellspacing="0" width="100%" class="">
                        ...
                        { 
                            ...
                            $checksub = CbscExamScores::model()->findByAttributes(array('exam_id'=>$_REQUEST['examid'],'student_id'=>$posts_1->id));
                            $exm = CbscExams::model()->findByAttributes(array('id'=>$_REQUEST['examid']));
							if($exm!=NULL)
							{
                            	$sub = Subjects::model()->findByAttributes(array('id'=>$exm->subject_id));
                    ...
                    if($i==1)
                    {
                        ...
                        $allscores = CbscExamScores::model()->findAllByAttributes(array('exam_id'=>$_REQUEST['examid']));
                        $sum=0;
    ...
    <?php
	$checkscores = CbscExamScores::model()->findByAttributes(array('exam_id'=>$_REQUEST['examid']));
	if($checkscores!=NULL)
	{
	?>
        ...
        <?php 
		$model1=new CbscExamScores('search');
	   ...
	   $examgroups = CbscExamGroups::model()->findByAttributes(array('id'=>$exm->exam_group_id)); 
            ...
            if($checkscores!=NULL)
            {
				...
				$new_array[]	= array(
                        ...
                        'buttons' => array(
									...
									'url'=>'Yii::app()->createUrl("/CBSCExam/examScores/update", array("sid"=>$data->id,"examid"=>$data->exam_id,"id"=>$_REQUEST["id"]))', // a PHP expression for generating the URL of the button
...
?>
<script>
$('input[type="text"][name="CbscExamScores[marks][]"]').blur(function(e) {
    if(isNaN($(this).val())){
  $(this).val('');
 }
});
...
$("form#exam-scores-form").submit(function(e) {
	...
	else
	{
		...
		$.ajax({
			url:'<?php echo Yii::app()->createUrl("/CBSCExam/examScores/create", array("id"=>$_REQUEST['id'], "examid"=>$_REQUEST['examid']));?>',
			type:'POST',
			data:data,
			dataType:"json",
