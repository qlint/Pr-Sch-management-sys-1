  ...
  if(isset($_REQUEST['id']) && isset($_REQUEST['examid']))
  {
    ...
    <div align="center" style="display:block; text-align:center !important;"><?php echo Yii::t('app','EXAM SCORES');?></div><br />
     <?php 
		...
		$semseter = Semester::model()->findByAttributes(array('id'=>$batch->semester_id));
    ...
    <!-- Course details -->
       ...
       <table style="font-size:14px;background:#DCE6F1;padding:10px 10px;border:#C5CED9 1px;">
            ...
            <tr>
                ...
                <td>
					<?php 
					if($exam->start_time!=NULL && $exam->start_time!='0000-00-00 00:00:00')
					{
			...
			<tr>
                ...
                <td>
					<?php 
					if($semseter->name!=NULL)
						echo ucfirst($semseter->name);
					else
						echo '-';
					?>
				</td>
           ...
           <?php } ?>
			   ...
			   $examgroups = CbscExamGroup17::model()->findByAttributes(array('id'=>$exm->exam_group_id));  
				...
				foreach($subject_cps as $subject_cp){
                 ...
                 <td style="width:250px;"><?php echo Yii::t('app','Total');?></td>
			...
			foreach($scores as $score)
			 ...
			 echo "<tr>";
				 ...
				 if(FormFields::model()->isVisible("fullname", "Students", "forStudentProfile")){
				 	echo "<td width='100'>".$student->studentFullName("forStudentProfile")."</td>";
                 }
				 $subject_marks	=	CbscExamScoresSplit17::model()->findAllByAttributes(array('exam_scores_id'=>$score->id));
				 foreach($subject_marks as $subject_mark)
				 {
				  	echo "<td width='100'>".$subject_mark->mark."</td>";
				 } 
				  echo "<td width='100'>".$score->total."</td>";
				 echo "<td width='100'>".$score->remarks."</td>";
				 echo "<td width='100'>";
