        ...
        <?php 
            ...
            ?>
				...
				if($exm!=NULL)
				{
					...
					$egup = CbscExamGroup17::model()->findByAttributes(array('id'=>$sub->elective_group_id));
					if($egup!=NULL)
                ...
                <div class="exam-table exam-table-line">
                            ...
                            if($checksub==NULL and (($sub->elective_group_id==0 and count($sub)!=0) or ($sub->elective_group_id!=0 and count($student_elective)!=0)))
                            {
                                ...
                                ?>
									...
									if($subject_cps !=NULL){
										if($insert_score == 1 and $flag==0)
										{
											echo $form->textField($model,'total['.$k.']',array('maxlength'=>3,'class'=>'total','readonly'=>true));
										}
										else
										{
											echo $form->textField($model,'total['.$k.']',array('maxlength'=>3,'class'=>'disabled_field','disabled'=>'disabled','class'=>'total','readonly'=>true));
										}
									}else{
										if($insert_score == 1 and $flag==0)
										{
											echo $form->textField($model,'total['.$k.']',array('maxlength'=>3,'class'=>'to_total'));
										}
										else
										{
											echo $form->textField($model,'total['.$k.']',array('maxlength'=>3,'class'=>'disabled_field','disabled'=>'disabled','class'=>'to_total'));
										}
									}
										?>
                                    </td>                        
                    ...
                    if($i==1)
                    {
                        ...
                        echo CHtml::link(Yii::t('app', 'Generate PDF'), array('exam17/splitpdf','id'=>$_REQUEST['id'],'examid'=>$_REQUEST['examid']),array('target'=>"_blank",'class'=>'pdf_but'));
	...
	?>
        ...
        <div style="position:relative">    
            <div class="edit_bttns" style="width:250px; top:-10px; right:-101px;">
                <ul>
                    <li>
                    <?php echo CHtml::link('<span>'.Yii::t('app','Clear All Scores').'</span>', "#", array('submit'=>array('exam17/deleteallSplit','id'=>$_REQUEST['id'],'examid'=>$_REQUEST['examid']), 'confirm'=>Yii::t('app','Are you sure you want to delete all scores ?.'), 'csrf'=>true,'class'=>'addbttn last'));?>
                    </li>
                </ul>
                <div class="clear"></div>
            </div>
            ...
            if($checkscores!=NULL)
            {
				...
				$new_array[]	= array(
                        ...
                        'buttons' => array(
									...
									'url'=>'Yii::app()->createUrl("/CBSCExam/exam17/updateSplit", array("sid"=>$data->id,"examid"=>$data->exam_id,"id"=>$_REQUEST["id"]))', // a PHP expression for generating the URL of the button
								  
									),
									'delete' => array(
									'label' => Yii::t('app','Delete'), // text label of the button									
									'url'=>'Yii::app()->createUrl("/CBSCExam/exam17/deleteSplit", array("id"=>$data->id))', // a PHP expression for generating the URL of the button
...
$("form#exam-scores-form").submit(function(e) {
	...
	else
	{
		...
		$.ajax({
			url:'<?php echo Yii::app()->createUrl("/CBSCExam/exam17/examScoresSplit", array("id"=>$_REQUEST['id'], "examid"=>$_REQUEST['examid']));?>',
			type:'POST',
			data:data,
			dataType:"json",
