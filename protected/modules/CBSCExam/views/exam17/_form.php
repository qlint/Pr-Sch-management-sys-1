...
function new_1(id)
{
	...
	if(!isNaN(val))
	{
		...
		for(i=1;i<=id;i++)
		{
	    	...
	    	document.getElementById('CbscExams17_maximum_marks_'+i).value = val;
		}
	}
	else
	{
...
function old_1(id)
{
	...
	if(!isNaN(val))
	{
		...
		for(i=1;i<=id;i++)
		{
			document.getElementById('CbscExams17_minimum_marks_'+i).value = val;
		}
	}
	else
	{
...
<?php 
$check = CbscExamGroup17::model()->findByAttributes(array('id'=>$_REQUEST['exam_group_id'],'batch_id'=>$_REQUEST['id']));
if($check!=NULL)
{ ?>
	<?php
                    ...
                    if(isset($_REQUEST['id']))
                    {
                        ...
                        if(count($posts)!=0)
                        {
                            ...
                            foreach($posts as $posts_1)
                            {
                                ...
                                $checksub = CbscExams17::model()->findByAttributes(array('exam_group_id'=>$_REQUEST['exam_group_id'],'subject_id'=>$posts_1->id));
                                if($checksub==NULL)
                                {
                                    if($j==0)
                                    {
		...
		*/
		array(
			...
			'buttons' => array(
                                                        ...
                                                        'url'=>'Yii::app()->createUrl("CBSCExam/exam17/update", array("sid"=>$data->id,"exam_group_id"=>$data->exam_group_id,"id"=>$_REQUEST["id"]))', // a PHP expression for generating the URL of the button
		...
		array(
					...
					'buttons' => array(					
						...
						'url'=>'Yii::app()->createUrl("CBSCExam/exam17/examScores/", array("examid"=>$data->id,"id"=>$_REQUEST["id"]))', // a PHP expression for generating the URL of the button
						)
					),
                   'template' => '{add}',
...
<br />
	<script>
$('input[type="text"][name="CbscExams17[maximum_marks][]"], input[type="text"][name="CbscExams17[minimum_marks][]"],input[type="text"][name="ElectiveExams[maximum_marks][]"],input[type="text"][name="ElectiveExams[maximum_marks][]"]').blur(function(e) {
    if(isNaN($(this).val())){
  $(this).val('');
 }
});
...
$("form#exams-form").submit(function(e) {
	...
	if (textBox == "") {
		...
		$.ajax({
			url:'<?php echo Yii::app()->createUrl("/CBSCExam/exam17/create", array("id"=>$_REQUEST['id'], "exam_group_id"=>$_REQUEST['exam_group_id']));?>',
			type:'POST',
			data:data,
			dataType:"json",
