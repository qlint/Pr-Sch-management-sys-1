...
?>
   ...
   ?>
    ...
    <div align="center" style="text-align:center; display:block;"><?php echo Yii::t('app','SEMESTER ASSESSMENT REPORT'); ?></div><br />
    <?php $student=Students::model()->findByAttributes(array('id'=>$_REQUEST['id'],'is_deleted'=>0,'is_active'=>1));
	$exams_alls = Exams::model()->findAll('exam_group_id=:x',array(':x'=>$_REQUEST['exam_group_id'])); // Selecting exams(subjects) in an exam group
	$allstatus	= 0;
	if($exams_alls!=NULL){
		foreach($exams_alls as $exams_all){
			$allmin 	 = $exams_all->minimum_marks;
			$subject = Subjects::model()->findByAttributes(array('id'=>$exams_all->subject_id));
			if($subject!=NULL){ 
				$score = ExamScores::model()->findByAttributes(array('exam_id'=>$exams_all->id,'student_id'=>$student->id));
				if($score!=NULL){
    ...
    <table style="font-size:13px; background:#DCE6F1; padding:10px 10px;border:#C5CED9 1px solid;">
            ...
            <tr>
				...
				$semester	=	Semester::model()->findByPk($_REQUEST['semid']);
...
<table width="100%" cellspacing="0" cellpadding="0" class="assessment_table">
  <tr class="tablebx_topbg" style="background-color:#DCE6F1;">
	...
	<?php }
		...
		{
			?>
			<th style="width:130px;"><?php echo Yii::t('app','Grade');?></th>
			<?php
		}
		?>
