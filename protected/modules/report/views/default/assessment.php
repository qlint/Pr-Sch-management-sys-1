...
<label><?php echo Yii::t('app','Academic Year');?> <span class="required">*</span></label>
				<?php
...
<div class="txtfld-col">
 ...
 <label><?php echo Yii::t('app','Select Semester'); ?></label>
<?php 
...
<div class="txtfld-col">
<label><?php echo Yii::app()->getModule('students')->fieldLabel("Students", "batch_id");?><span class="required">*</span></label>
<?php 
			
                        
                        if(isset($year_id) && $year_id==NULL)
                        {
                ...
                {
                    if(Yii::app()->user->hasState('ses_session_yr')){
...
<div class="txtfld-col-box">
<div class="txtfld-col">
<label><?php echo Yii::t('app','Examination');?><span class="required">*</span></label>
					<?php 
                    if($batch_id)
                    {
					...
					<div class="required" id="examgroup_error"></div>
</div>
</div>
<div class="button-bg">
...
<ul>                                    
<li><?php echo CHtml::submitButton(Yii::t('app','Search'),array('name'=>'search','class'=>'formbut', 'id'=>'search_btn')); ?></li>                                   
</ul>
</div> 
								...
								<tr>
									...
									<td style="border:none;<?php if($score->is_failed == 1){?>color:#F00;<?php }?>">
										...
										<?php 
										 ...
										 $grades = GradingLevels::model()->findAllByAttributes(array('batch_id'=>$examgroup->batch_id),array('order'=>'min_score DESC'));
...
$('#search_btn').click(function(ev){
	...
	$('#examgroup_error').html('');

	var academic_yr = $('#yid').val();

	var batch_id	= $('#batch').val();
	
	var group_id	= $('#exam_id').val();
	...
	if(group_id == ''){
		...
		$('#examgroup_error').html('<?php echo Yii::t('app','Examgroup cannot be blank'); ?>');
