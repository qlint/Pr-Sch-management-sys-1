...
public function actionSettime()
               ...
               { 	  
				   if(isset($_POST['elective_id']) && $_POST['elective_id']!= NULL)
                       ...
                       { 
						   ...
						   //check the value exit 
						   
						   /*$vals = array_count_values($elective_array); 
							$index	=	$vals[$_POST['employee_id'][$key]]; 
							 if($key!=0 and $index>1){												 
								 $keys= "employee_id".$key;
								   $errors[$keys][]	= 'Teacher is already selected.';
								   echo CJSON::encode(array(
										'status'=>'error',
										'errors'=>$errors                                                                        
										));
				...
				foreach($electives as $elective)
				{
					...
					echo CHtml::openTag('tr');
						...
						foreach($data as $value=>$name)
						{
							...
							$options .= CHtml::tag('option',array('value'=>$emp_name->id),Employees::model()->getTeachername($emp_name->id),true);
						}
					...
					if($flag==$count){
						foreach($_POST['elective_id'] as $key => $val){
							...
							if(isset($_POST['employee_id'][$key]) AND $_POST['employee_id'][$key]!=NULL){
								...
								$elective_array	= $_POST['TimetableEntries']['employee_id'];
								$vals = array_count_values($elective_array); 
								$index	=	$vals[$_POST['TimetableEntries']['employee_id'][$key]]; 
							}
	...
	{ 
		...
		if($subject->elective_group_id != 0){ 
			...
			if($group){
				$electives =  Electives::model()->findAllByAttributes(array('elective_group_id'=>$group->id,'batch_id'=>$group->batch_id, 'is_deleted'=>0));
