...
public function   init() {
  ...
  private function registerAssets(){
	...
	protected function performAjaxValidation($model)
	{
		...
		if(isset($_POST['batch_id']) and $_POST['batch_id']==0) // from settings module
		{ 
			$this->renderPartial('_ajax_form', array('model'=>$model,'batch_id'=>$_POST['batch_id']), false, true);
		}
		else               // from courses module
		{ 
  ...
  public function actionAjax_Create(){
		...
		{ 
		   	$model=new Subjects;
			$model->attributes=$_POST['Subjects']; 
			if(isset($_POST['Subjects']['subject_tilte1']) and $_POST['Subjects']['subject_tilte1'] != NULL){
				$sub1					= $_POST['Subjects']['subject_tilte1'];
				$model->subject_tilte1	= $_POST['Subjects']['subject_tilte1'];
			}
			else{
				$sub1	= '';
				$model->subject_tilte1	= '';
			}
			if(isset($_POST['Subjects']['subject_tilte2']) and $_POST['Subjects']['subject_tilte2'] != NULL){
				$sub2					= $_POST['Subjects']['subject_tilte2'];
				$model->subject_tilte2	= $_POST['Subjects']['subject_tilte2'];
			}
			else{
				$sub1	= '';
				$model->subject_tilte2	= '';
			}			
			$split	=	array('0'=>$sub1,'1'=>$sub2);
			if(isset($_POST['Subjects']['split_subject']) and $_POST['Subjects']['split_subject'] != NULL){ 
				$model->split_subject		=	$_POST['Subjects']['split_subject'];
			}
			else{
     ...
     public function actionAjax_delete(){
				 ...
				 CbscExams::model()->deleteAllByAttributes(array('subject_id'=>$id));
				  TimetableEntries::model()->deleteAllByAttributes(array('subject_id'=>$id,'is_elective'=>0));
				 $Exams = Exams::model()->findAllByAttributes(array('subject_id'=>$id));
							if($Exams){
