  ...
  private function registerAssets(){
	...
	public function actionCreate()
	{
		...
		{	
			...
			if($configuration->config_value == -1){				 
				$model->exam_format = $course->exam_format;
			}
				...
				if(!isset($_POST['Batches']['duplicate']) or (isset($_POST['Batches']['duplicate']) and $_POST['Batches']['duplicate'] == 0)){ 
					...
					if($common_subjects){
						foreach($common_subjects as $common_subject){
							...
							$model_2->split_subject	=	$common_subject->split_subject;
							if($model_2->split_subject == 1){
								$model_2->subject_tilte1	= "a";
								$model_2->subject_tilte2	=	"a";
							}
							if($model_2->save())
							{ 
								//duplicate commen subject
								//$subject_split_arr		= array();		
								...
								foreach($all_comsubjects_splits as $all_comsubjects_split){
									...
									/*if($new_comsubjects_split->save()){
											$subject_split_arr[$all_comsubjects_split->id]	= $new_comsubjects_split->id;
										}*/
									
								}
								/*if($common_subject->split_subject == 1 and $all_comsubjects_splits != NULL){									
										$subject_arr[$common_subject->id]			= $subject_split_arr;
									}*/
							}
						}
					}
		...
		if(isset($_POST['Batches']['duplicate']) and $_POST['Batches']['duplicate'] == 1){					
			...
			//duplicate batch
			$examformat	= ExamFormat::model()->getExamformat($model->id);
			if($_POST['Batches']['subject']==1 or $_POST['Batches']['all']==1){	//duplicate subject					
					...
					if($all_subjects){
						...
						foreach($all_subjects as $all_subject){
							...
							$new_sub_entry->split_subject	=	$all_subject->split_subject;
							...
							if($new_sub_entry->split_subject == 1){
								$new_sub_entry->subject_tilte1	= "a";
								$new_sub_entry->subject_tilte2	=	"a";
							}
							
							if($new_sub_entry->save()){
								//Duplicate Subject Split details
								if($examformat == 1){ // split only need if new batch is in default exam format
									$subject_split_arr		= array();							    								
									...
									foreach($all_subjects_splits as $all_subjects_split){
										...
										if($new_subjects_split->save()){
											$subject_split_arr[$all_subjects_split->id]	= $new_subjects_split->id;
										}
									}
									if($all_subject->split_subject == 1 and $all_subjects_splits != NULL){									
										$subject_arr[$all_subject->id]			= $subject_split_arr;
									}
								}
								//End Duplicate Subject Split details
			...
			//subject end 
			if($_POST['Batches']['electives']==1){	//duplicate electives subject	
				if($examformat == 1){ //Electives only need if new batch is in default exam format						
			...
			//elective end
			if($_POST['Batches']['classtimimg']==1 or $_POST['Batches']['all']==1){	//duplicate classtimimg
				$all_classtime = ClassTimings::model()->findAllByAttributes(array('batch_id'=>$_POST['Batches']['batch_list']));
				foreach($all_classtime as $classtime){
					$new_classtime				=	new ClassTimings;
					$new_classtime->attributes 	= $classtime->attributes;
					$new_classtime->batch_id 	= $model->id;
					if($new_classtime->save()){
						//time table
							if($_POST['Batches']['timetable']==1 or $_POST['Batches']['all']==1){	//duplicate classtimimg
								
								if($examformat == 1){
									$class_timetables	=	TimetableEntries::model()->findAllByAttributes(array('class_timing_id'=>$classtime->id,'batch_id'=>$_POST['Batches']['batch_list']));
								}
								else{
									$class_timetables	=	TimetableEntries::model()->findAllByAttributes(array('class_timing_id'=>$classtime->id,'batch_id'=>$_POST['Batches']['batch_list'], 'split_subject'=>0));
								}
								foreach($class_timetables as $class_timetable){
									
									$new_timetable					=	new TimetableEntries;
									$new_timetable->attributes 		= 	$class_timetable->attributes;
									
									if($_POST['Batches']['teacher_duplicate'] == 0){
									 $new_timetable->employee_id       = NULL;
									}
									$new_timetable->batch_id 		= 	$model->id;
									$new_timetable->class_timing_id	= 	$new_classtime->id;									
									if($class_timetable->is_elective == 2){
										$new_timetable->is_elective=2;
										$subject_id	=	$elective_subject[$class_timetable->subject_id];
									}
									else{
										$subject_id	=	$normal_subject[$class_timetable->subject_id];
									}									
									$new_timetable->subject_id 		= $subject_id;
									if($class_timetable->split_subject != 0){
										$new_timetable->split_subject	= $subject_arr[$class_timetable->subject_id][$class_timetable->split_subject];
									}
									else{
										$new_timetable->split_subject	= 0;
									}
									$new_timetable->save();
        ...
        public function actionSemesters()
        {
            ...
            {     
				...
				if($sem_status == 1){
					...
					$criteria->addNotInCondition('id', array($_REQUEST['id']));              
					...
					//$data = CHtml::listData(Batches::model()->findAllByAttributes(array('course_id'=>$_REQUEST['course_id'],'is_active'=>1,'is_deleted'=>0)),'id','name');               	
					foreach($data as $value=>$name){
							$batches	.= CHtml::tag('option', array('value'=>$value), CHtml::encode(html_entity_decode($name)),true);
					}
				}
		...
		{  
			if($_POST['semester_id'] == 0){
				...
				echo CHtml::tag('option',array('value'=>$value),CHtml::encode(html_entity_decode($name)),true);
                    ...
                    $data = Batches::model()->findAll('academic_yr_id=:x AND is_deleted=:y AND is_active=1 AND semester_id=:sem_id AND course_id=:course_id AND id!=:p ',array(':p'=>$_POST['id'],':x'=>$_POST['year'],':y'=>0,':sem_id'=>$_POST['semester_id'],':course_id'=>$course_id));				
                    echo CHtml::tag('option', array('value' => 0), CHtml::encode(Yii::t('app','Select')), true);
                    $data=CHtml::listData($data,'id','coursename');
                    foreach($data as $value=>$title)
                    {
						...
						if(isset($_POST['elective_premote']) and $_POST['elective_premote'] == 1){
							if($premot_succes	==	1){
								...
								if(isset($elective_new->id) and $elective_new->id!=NULL){
									$alreday_ex		=	StudentElectives::model()->findAllByAttributes(array('elective_group_id'=>$elective_new->elective_group_id,'student_id'=>$sid,'batch_id'=>$new_batch_id));									
	...
	public function actionAddnew() {
	   	...
	   	if(isset($_POST['Submit']))
        {
			...
			$model->save();	
					if($model->save() and $model->exam_format == 2){
        ...
        public function actionAddStudents()
	{
		...
		$criteria->condition    =   't.is_deleted=:is_del AND t.is_active=1 AND bs.batch_id<>:cbatch AND bs.`status`=1';   
		...
		if(isset($_REQUEST['cid']) and $_REQUEST['cid']!=NULL and isset($_REQUEST['Students']['batch_id']) and $_REQUEST['Students']['batch_id']!=NULL  and $_REQUEST['Students']['batch_id']!=0)
		{
			...
			$criteria->condition=$criteria->condition.' and '.'bs.batch_id = :batch_id';
	...
	{		
			...
			$criteria	= new CDbCriteria; 
			$criteria->condition            = 	'`course_id`=:course_id AND `is_deleted`=0 AND `is_active`=1' ;
			$criteria->params				= 	array(':course_id'=>$_POST['cid']);
			$criteria->order				= 	'name ASC';  
			//$criteria->params				= 	array(':course_id'=>$_POST['cid']); 
			$criteria->addNotInCondition('id', array($_REQUEST['bid'])); 
			$data   						= 	Batches::model()->findAll($criteria);   
        ...
        public function actionAdd_all()
	{
        ...
        public function actionCheckBatch(){
            if(isset($_POST['batch_id'])){
                $format = ExamFormat::model()->getExamformat($_POST['batch_id']);
                echo CJSON::encode(array('status'=>'success','format'=>$format));
                exit;	
            }else{
                echo CJSON::encode(array('status'=>'error'));
                exit;
            ...
            if(Yii::app()->request->isPostRequest){ 
                ...
                { 
                    ...
                    if($model->save())
                    {
                        ...
                        $check_model        =   BatchStudents::model()->findAllByAttributes(array('student_id'=>$_REQUEST['sid'],'status'=>1,'result_status'=>0,'batch_id'=>$_REQUEST['id']));
                        ...
                        if($check_model==NULL)
                        {
							...
							$batch_active	 = BatchStudents::model()->findAllByAttributes(array('student_id'=>$_REQUEST['sid'],'status'=>1,'result_status'=>0)); 
							if($batch_active == NULL) 
							{                                                                                 
		...
		public function actionCheckElective()
		{
			$batch_id	= $_REQUEST['batch_id'];
			$is_cbse	= ExamFormat::model()->getExamformat($batch_id);
			echo $is_cbse;
		}
		public function actionCheckCoursecbsc()
		{
			$course 	= Courses::model()->findByAttributes(array('id'=>$_REQUEST['course_id']));
			echo $course->exam_format;
		}
}
