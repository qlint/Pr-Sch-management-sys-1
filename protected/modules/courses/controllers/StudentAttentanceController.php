...
class StudentAttentanceController extends RController
{
	...
	public function actionIndex()
	{
		if(Configurations::model()->studentAttendanceMode() != 2){
			...
			$this->redirect(array('/courses/studentSubjectAttendance/daily', 'id'=>$_REQUEST['id']));
		}
	}
    ...
    {        
		...
		Yii::app()->osPdf->generate("application.modules.courses.views.studentAttentance.dailyattendancepdf", $filename, array(), 1); 
	}
	...
	public function actionSendsms() // Function to send Attendance SMS to all students of a batch
	{
		if(Yii::app()->request->isPostRequest){ 
					...
					foreach($students as $student){
						...
						if($guardian != NULL){ // Check if guardian added							 
							...
							if($to!='' and $notification->sms_enabled=='1' and $notification->parent_1=='1') // If absent and phone number is provided, send SMS								
							{
								...
								$message	= str_replace("<Date>",$date,$message);
								//$is_absent  = StudentAttentance::model()->findByAttributes(array('student_id'=>$students->id,'date'=>$date));	
			...
			if(isset($_REQUEST['flag']) and ($_REQUEST['flag']==1)){
				$this->redirect(array('/courses/studentAttentance','id'=>$_REQUEST['batch_id'],'date'=>$_REQUEST['date']));
			}
			else{
				$this->redirect(array('/courses/studentAttentance/monthlyAttendance','id'=>$_REQUEST['batch_id']));
			}			
		}
		else{
	...
	// student Attendace fron students tab inside batch 
	public function actionStudentLeave(){
		
		$model	= new StudentAttentance;
		
		if(isset($_POST['StudentAttentance'])){
			$student_id	= 	$_POST['StudentAttentance']['student_id'];
			$batch_id	=	$_POST['StudentAttentance']['batch_id'];
			$date		=	date('Y-m-d',strtotime($_POST['StudentAttentance']['date']));	
			$model = StudentAttentance::model()->findByAttributes(array('student_id'=>$student_id,'batch_id'=>$batch_id,'date'=>$date));
		...
		$this->renderPartial('mark-leave',array('model'=>$model),false,true);		
	}
