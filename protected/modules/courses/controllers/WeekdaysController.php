...
class WeekdaysController extends RController
{
	...
	public function beforeAction(){
		...
		if(in_array($action, $redirect_actions)){
			if(Configurations::model()->timetableFormat(isset($_REQUEST['id'])?$_REQUEST['id']:NULL)==2){ // timetable format is not selected as course level					
				$params	= array('/courses/flexible/'.$action);
	...
	public function actionIndex()
	{
		...
		if(isset($_POST['Weekdays']))
		{
			...
			$weeks = $model->findAll("batch_id=:x", array(':x'=>$_REQUEST['id']));
			foreach($weeks as $week_arr)
			{
				$week_array[] = $week_arr->weekday;
			}
			$class_timing = ClassTimings::model()->findAllByAttributes(array('batch_id'=>$_REQUEST['id']));
			foreach($class_timing as $class)
			{ 
				if($week_array[0]!= 1)
				{
					$class->on_sunday = 0;
			...
			$criteria->addNotInCondition('weekday_id',$week_array);
			$datas = TimetableEntries::model()->findAll($criteria);
    ...
    {        
		...
		$filename 	= $batch_name->name.Yii::t('app',' Class Timetable').'.pdf';		
		Yii::app()->osPdf->generate("application.modules.courses.views.weekdays.exportpdf", $filename, array(), 1);
	}
