		...
		{	
			...
			if($model->save()){  
				...
				if(isset($_POST['SubjectsCommonPool']['id']) and $_POST['SubjectsCommonPool']['id']!=NULL){
					...
					if($old_course != $model->course_id)

					{

						$sub_batch = Subjects::model()->findAllByAttributes(array('admin_id'=>$model->id,'is_edit'=>0));

						foreach($sub_batch as $sub_batchs)
