...
<?php 
      ...
      $format = ExamFormat::model()->getExamformat($_REQUEST['id']);
 ?>
<?php
...
<li><?php echo CHtml::ajaxLink('<span>'.Yii::t('app','Add New').' '. Yii::app()->getModule('students')->fieldLabel("Students", "batch_id").'</span>',$this->createUrl('batches/Addnew'),array('onclick'=>'$("#jobDialog").dialog("open"); return false;','update'=>'#jobDialog','type' =>'GET','data' => array( 'val1' =>$batch->course_id ),'dataType' => 'text'),array('id'=>'showJobDialog1','class'=>'a_tag-btn')); ?></li>
                              ...
                              <div id="promote_show" style="color:#F00"> * <?php echo Yii::t('app','Assign Elective');?> : <?php echo Yii::t('app','Promoted batch should have the same assigned elctive name.');?></div>                      <?php
								}
								?>
                                            ...
                                            <tr>                                                
                                                <td width="14%">
                                                    ...
                                                    <?php 
                                                        ...
                                                        echo CHtml::dropDownList('action', (isset($_POST['action']))?$_POST['action']:'', $options,array('id'=>'action_drop','encode'=>false,'prompt'=>Yii::t('app','Select'))); 
                                                    ?>
                                                </td>        
                                                <td  width="1%"></td>
                                                <td width="14%">
                                                ...
                                                <td>
                                                    ...
                                                    echo CHtml::dropDownList('semester_id',(isset($_POST['semester_id']))?$_POST['semester_id']:'',$data,array('prompt'=>Yii::t('app','Select'),																					'encode'=>false,
                                                                                    'ajax' => array(
                                                                                    'type'=>'POST',
                                            ...
                                            <td  width="1%"></td>
                                                ...
                                                <td  width="14%">
                                                    ...
                                                    {                                                        
                                                        ...
                                                        $data = Batches::model()->findAll('academic_yr_id=:x AND is_deleted=:y AND is_active=1 AND semester_id=:sem_id AND course_id=:course_id AND id!=:p',array(':p'=>$_REQUEST['id'],':x'=>$_POST['year'],':y'=>0,':sem_id'=>$_POST['semester_id'],':course_id'=>$course_id));				                                                        
                                                    ...
                                                    echo CHtml::dropDownList('batch_id',(isset($_POST['batch_id']))?$_POST['batch_id']:'',$data,array('encode'=>false,'prompt'=>Yii::t('app','Select'),'id'=>'batch_id','disabled'=>(isset($_POST['action']) and ($_POST['action']==-1 or $_POST['action']==1))?false:true)); 
												 ...
												 ?>
                                                	<td id="elective_pre">
													<?php  //if($_REQUEST['id']!="" && ExamFormat::model()->getExamformat($_REQUEST['id'])== 1){?>
																<?php echo CHtml::checkBox('elective_premote',0,array());  ?><?php echo  Yii::t('app','Assign Elective');?>
													 <?php //} ?>
													</td>
                                                    <?php
												   }
                                            ...
                                            <div class="pdtab_Con" style="padding-top:0px;">
                                                <table width="100%" cellspacing="0" cellpadding="0" border="0">
                                                    <tr class="pdtab-h">
                                             		...
                                             		<tr>
                                                        <td colspan="5" style="padding:8px 0 8px 20px;" >
...
$("#batch_id").change(function(){
	if($(this).val() != 0){	
		...
		var batch_id	= '<?php echo $_REQUEST['id']; ?>';
	}
		
	if(batch_id != ''){
		$.ajax({
			type: "POST",
			url: <?php echo CJavaScript::encode(Yii::app()->createUrl('courses/batches/checkElective'))?>,
...
{    
    if(ch.checked)
    { 
            $('.promote_check').prop('checked', true);
    }
    else
    {
            $('.promote_check').each(function() { //loop through each checkbox
...
{
	var numberOfChecked = $('.promote_check:checked').length; //count of all checked checkboxes with class "checkbox1"
	var totalCheckboxes = $('.promote_check:checkbox').length; //count of all textboxes with class "checkbox1"
