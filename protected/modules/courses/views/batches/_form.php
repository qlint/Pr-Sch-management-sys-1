...
<tr>
 ...
 <tr>       
    ...
    <tr>
    <td width="45%" class="timetable_formats"><div><?php 
	if($model->exam_format ==NULL){
		$model->exam_format=1;
	}
	echo $form->radioButton($model, 'exam_format', array('value'=>'1','uncheckValue'=>null))." ".CHtml::label(Yii::t('app',"Default"));?>
    <br />
	<?php 
      ...
      <td colspan="4">
	    <table class="popup-box" width="100%">
            ...
            <tr class="batch_list_block">
                ...
                <div class="batch_list_block_elective">
            ...
            <tr class="teacher_duplicate">
            <td>
            	<?php echo $form->checkBox($model,'teacher_duplicate',array('class'=>'duplicatecheck')); ?><?php echo Yii::t('app','Teachers time schedule is required');?>
...
$('#Batches_batch_list').change(function(){
	
    var batch_id  =$("#Batches_batch_list").val();  
    $(".batch_list_block_elective").show();  
    $.ajax({
                    type: "POST",
                    url: <?php echo CJavaScript::encode(Yii::app()->createUrl('courses/batches/checkBatch'))?>,
                    ...
                    success: function(response){						
                        if(response.status=="success"){
                            if(response.format==2){
                                $(".batch_list_block_elective").hide();  
                            }
                        }                        
                    }
...
$('#Batches_duplicate').change(function(ev){
	var radioValue = $("input[name='Batches[exam_format]']:checked").val();
	if(radioValue == 1)
	{
		$('.batch_list_block_elective').show();
	}
...
$(document).ready(function() {
    $('input[type=radio][name=Batches[exam_format]]').change(function() {
        if (this.value == 1) {
            $('.batch_list_block').hide();
			$('.teacher_duplicate').hide();
			$('#Batches_duplicate').prop('checked', false);
        }
...
$('#Batches_duplicate').change(function(ev){
	if(this.checked == true){
		...
		if($('#Batches_timetable').prop('checked') == true){
			$('.teacher_duplicate').show();
		}
