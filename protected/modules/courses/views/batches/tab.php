        ...
        <div class="cb_left">
            <ul>
                ...
                if($sem_enabled==1 and $batch->semester_id!=NULL){
					...
					echo  Yii::t('app','Course').' / '.Yii::app()->getModule('students')->fieldLabel("Students", "batch_id").' / '.Yii::t('app','Semester').' : '; ?></strong> <?php echo html_entity_decode(ucfirst($coursename)); ?> / <?php echo html_entity_decode(ucfirst($batchname)); ?> / <?php echo html_entity_decode(ucfirst($semester->name));  
                }else{
					echo Yii::t('app','Course').' / '.Yii::app()->getModule('students')->fieldLabel("Students", "batch_id").' : '; ?></strong> <?php echo  html_entity_decode(ucfirst($coursename)); ?> / <?php echo  html_entity_decode(ucfirst($batchname)); 
				}
				?></li>
            ...
            <div class="act_drop" id="tabdropx">
				...
				?>
                  ...
                  <?php  if($batch_id!="" && ExamFormat::model()->getExamformat($batch_id)== 2)
                     ...
                     <li class="exams"><?php echo CHtml::link(Yii::t('app','Exams').'<span>'.Yii::t('app','for add new exam').'</span>', array('/CBSCExam/exam','id'=>$batch_id)) ?></li>
                     	<?php } 
						else
						{?>
							<li class="exams"><?php echo CHtml::link(Yii::t('app','Exams').'<span>'.Yii::t('app','for add new exam').'</span>', array('/examination/exam','id'=>$batch_id)) ?></li>
				  <?php }?>
               		 ...
               		 <li class="newsub"><?php echo CHtml::link(Yii::t('app','New Subject').'<span>'.Yii::t('app','for add new subject').'</span>', array('#'),array('id'=>'add_subject-name-side')) ?></li>
                         <?php if(ExamFormat::model()->getExamformat($_REQUEST['id'])!=2){ ?>
                ...
                <li class="assign_tea"><?php echo CHtml::link(Yii::t('app','Edit / Assign Teacher').'<span>'.Yii::t('app','Edit / assign class teacher').'</span>', array('#'),array('id'=>'assign_class_teacher')) ?></li>
                <?php
				}
				?>
	...
	?>
        <div class="pagetab-bg-tag-a">
            <ul>
                ...
                <?php     
                if(Yii::app()->controller->id=='coScholastic')
                {
                	echo '<li class="active">'.CHtml::link(Yii::t('app','Co-Scholastic'), array('/courses/coScholastic','id'=>$_REQUEST['id'])).'</li>';
                }
                else
                {
                	echo '<li>'.CHtml::link(Yii::t('app','Co-Scholastic'), array('/courses/coScholastic','id'=>$_REQUEST['id'])).'</li>';
                }
                ?>
				...
				$model = AttendanceSettings::model()->findByAttributes(array('config_key'=>'type'));
					...
					if(Yii::app()->controller->id=='studentAttentance' or Yii::app()->controller->id=='studentSubjectAttendance')
					{
						if(Configurations::model()->studentAttendanceMode() == 2){
							echo '<li class="active">'.CHtml::link(Yii::t('app','Attendances'), array('/courses/studentSubjectAttendance/daily','id'=>$_REQUEST['id'])).'</li>';
						}
						else{
					...
					{
						if(Configurations::model()->studentAttendanceMode() == 2){
							echo '<li>'.CHtml::link(Yii::t('app','Attendances'), array('/courses/studentSubjectAttendance/daily','id'=>$_REQUEST['id'])).'</li>';
						}
						else{
