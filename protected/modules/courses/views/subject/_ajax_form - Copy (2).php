   ...
   ($model->isNewRecord)?CController::createUrl('subject/ajax_create')
    ...
    <div class="row"> <?php echo $form->labelEx($model,'batch_id', array('style'=>'color:#000000')); ?>
      <?php 
			...
			echo $form->dropDownList($model, 'batch_id', CHtml::listData(Batches::model()->findAll($criteria),'id','coursename'),array('onchange'=>'checksplit()', 'id'=>'batch_id','encode'=>false,'prompt'=>Yii::t('app','Select')));
			?>
	...
	<?php echo $form->textField($model,'max_weekly_classes',array('maxlength'=>'3')); ?> 
	  ...
	  <?php if((isset($batch_id) and $batch_id==0) or ($_REQUEST['batch_id'] != NULL and ExamFormat::model()->getExamformat($_REQUEST['batch_id'])== 1)){ ?>
