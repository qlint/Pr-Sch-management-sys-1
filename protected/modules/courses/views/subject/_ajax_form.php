   ...
   ($model->isNewRecord)?CController::createUrl('subject/ajax_create')
    ...
    <div class="row"> <?php echo $form->labelEx($model,'batch_id', array('style'=>'color:#000000')); ?>
      <?php 
			...
			echo $form->dropDownList($model, 'batch_id', CHtml::listData(Batches::model()->findAll($criteria),'id','coursename'),array('onchange'=>'checksplit()','encode'=>false,'prompt'=>Yii::t('app','Select')));
			?>
	...
	<?php echo $form->textField($model,'max_weekly_classes',array('maxlength'=>'3')); ?> 
	  ...
	  <?php
	   if($_REQUEST['update_id'] != NULL){ 
		   $sub = Subjects::model()->findByAttributes(array('id'=>$_REQUEST['update_id']));
		   $batch = Batches::model()->findByAttributes(array('id'=>$sub->batch_id));
	   }
	  ?>
	  <?php if((isset($batch_id) and $batch_id==0) or ($_REQUEST['batch_id'] != NULL and ExamFormat::model()->getExamformat($_REQUEST['batch_id'])== 1) or ($_REQUEST['update_id'] != NULL and ExamFormat::model()->getExamformat($batch->id)== 1)){ ?>
      <div class="fancy-box-block fancy-box-block-checkd" id="split_div">
         ...
         echo $form->checkBox($model,'split_subject'); ?>
				...
				?>
                <div class="split" style="display:none" id="category_div">
					<?php
                    for($i=1;$i<=2;$i++){?>            
                        <div class="row"> <?php echo $form->labelEx($model,'subject_tilte'.$i,array('style'=>'color:#000000')); ?>&nbsp;<span class="required">*</span>
                            ...
                            <?php }else{
								?>
                                 <div class="input-notification-error  error-simple png_bg" style=" display:none" id="error2"><?php echo Yii::t('app','Second Sub Category cannot be blank.');?></div>
                                <?php
							}
								?>
                            </div>
...
<script>
function checksplit(){ 
	var batch_id = $('#Subjects_batch_id').val();
	if(batch_id != ''){
		$.ajax({
			type: "POST",
			...
			success: function(result){						
				if(result == 2){ // cbsc					
					$('#Subjects_split_subject').prop('checked', false);
					$("#Subjects_subject_tilte1").val('');
					$("#Subjects_subject_tilte2").val('');
					$('#split_div').hide('slow');
					$('#category_div').hide('slow');
				}
				else{ // default
					if($('#Subjects_split_subject').is(":checked")){
						$('#Subjects_split_subject').prop('checked', true);
					}
					else{
						$('#split_div').show('slow');
						$('#Subjects_split_subject').prop('checked', false);
