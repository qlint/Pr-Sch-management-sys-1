...
class TeacherSubjectAttendanceController extends RController
{
	...
	public function actionIndex()
	{
		if(Configurations::model()->teacherAttendanceMode() != 1){
			...
			$this->redirect(array('/attendance/employeeAttendances'));
		}
	}	
	...
	public function actionRemove($id, $dept, $emp, $date){
		if(Yii::app()->request->isPostRequest){			
			$entry 	= TeacherSubjectwiseAttentance::model()->findByPk($id);
			if($entry!=NULL){
				$entry->delete();
			}			
			$this->redirect(array('index', 'dept'=>$dept, 'emp'=>$emp, 'date'=>$date));
		}
		else{
	...
	public function actionPdf(){
        ...
        Yii::app()->osPdf->generate("application.modules.attendance.views.teacherSubjectAttendance.pdf", $employee, array(), 1);
	}
}
