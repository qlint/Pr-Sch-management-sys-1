	...
	public function actionDelete_student()
	{
		if(Yii::app()->request->isPostRequest){
			...
			if($model->saveAttributes(array('is_active'=>0, 'is_deleted'=>1))){
				...
				$studentdocuments = StudentDocument::model()->findAllByAttributes(array('student_id'=>$id));
				if($studentdocuments){
					foreach($studentdocuments as $studentdocument)
					{
						$uploads[] =$studentdocument->id;
						$files[]   =$studentdocument->file;
					...
					foreach($uploads as $upload)
					{
						...
						{
							$updoc = DocumentUploads::model()->findByAttributes(array('file_id'=>$upload,'file_name'=>$file));
							if($updoc)
							{
								$updoc->delete();
							}
	 ...
	 public function actionManage()
	 {
		...
		{ 
			...
			$model->academic_yr = $_REQUEST['Students']['academic_yr'];
                        $active_status  =   ($_REQUEST['Students']['academic_yr']=='in')?1:0;
                        $criteria->join		= 'JOIN `batch_students` `t1` ON `t`.`id` = `t1`.`student_id`'; 
                        $criteria->condition	= $criteria->condition.' and (`t1`.`result_status`=:result_status and `t1`.`status`=:status)';                        
                        $criteria->params[':result_status']	= 0;
                        $criteria->params[':status']		= $active_status;                                                
		...
		{		
			...
			if($_REQUEST['Students']['status'] != 'all'){
				...
				$criteria->params[':status'] = $_REQUEST['Students']['status'];
                            ...
                            $criteria->condition =  $criteria->condition.' and t.is_active= (0 OR 1)';			                            
                        }
