...
class StudentsModule extends CWebModule
{
	...
	public function studentsOfBatch($batch_id, $is_active=1){
		...
		$criteria = new CDbCriteria;
		$criteria->join 	= "JOIN `batch_students` `bs` ON `bs`.`student_id`=`t`.`id`";
        $criteria->condition = '`t`.`is_deleted`=:is_deleted AND `t`.`is_active`=:is_active AND `bs`.`batch_id`=:batch_id AND `bs`.`status`=:status';
        $criteria->params[':is_deleted'] = 0;
		$criteria->params[':status']     = 1;
        $criteria->params[':is_active']  = $is_active;
        $criteria->params[':batch_id']   = $batch_id;
        ...
        if($is_active){
        	$criteria->condition 	.= " AND `bs`.`result_status`=:result_status";
	        $criteria->params[':result_status']  = 0;
	    }
		$criteria->order	= 't.first_name ASC';
        return Students::model()->findAll($criteria);
        ...
        public function inactiveStudents($batch_id){ 
            ...
            $criteria = new CDbCriteria;
			$criteria->join 	= "JOIN `batch_students` `bs` ON `bs`.`student_id`=`t`.`id`";
           // $criteria->condition = '`t`.`is_deleted`=:is_deleted AND `bs`.`batch_id`=:batch_id AND `bs`.`result_status`=:result_status AND `bs`.`status`=:status AND `bs`.`academic_yr_id`=:academic_yr_id AND `t`.`is_active`=:is_active';
		   $criteria->condition = '`t`.`is_deleted`=:is_deleted AND `bs`.`batch_id`=:batch_id AND `bs`.`result_status`=:result_status AND `bs`.`status`=:status AND `bs`.`academic_yr_id`=:academic_yr_id';
            $criteria->params[':is_deleted'] = 0;           
            $criteria->params[':batch_id']   = $batch_id;
			$criteria->params[':result_status']     = 0;
