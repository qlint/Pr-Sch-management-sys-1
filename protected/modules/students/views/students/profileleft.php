...
<div id="jobDialog"></div>
        ...
        <ul>
        	<?php
			 if(count($batchstudents) == 1){
				 foreach($batchstudents as $batches)
				 {
					 $batch_stud = BatchStudents::model()->findByAttributes(array('batch_id'=>$batches->id));
					if($batch_stud->result_status == 2)
					{?>
	...
	<?php if(count($batchstudents)>1){
		  ...
		  if(count($batchstudents) == 1){ 
			  ...
			  $course 			= 	Courses::model()->findByAttributes(array('id'=>$posts->course_id));
	...
	<?php $sem_enabled= Configurations::model()->isSemesterEnabledForCourse($course->id);?>		
			 <?php if($semester_enabled == 1 and $sem_enabled == 1 and $posts->semester_id!=NULL){ ?>
                <li>
                <span><?php echo '<strong>'.Yii::t('app','Semester').'&nbsp;:'.'</strong>';?>&nbsp;
                    <?php    $semester 	= Semester::model()->findByAttributes(array('id'=>$posts->semester_id));
