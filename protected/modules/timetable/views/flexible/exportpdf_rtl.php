...
?>
    ...
    <?php
     ...
     ?>
    <div align="center" style="display:block; text-align:center;"><?php 
		echo Yii::t('app','CLASS TIME TABLE');?> - 
		<?php
		 if($semester_enabled == 1 and $sem_enabled == 1 and $batch->semester_id!=NULL){
			$semester 	= Semester::model()->findByAttributes(array('id'=>$batch->semester_id)); 
			echo $course_name->course_name; ?> (<?php echo $batch->name.' , '.$semester->name;
		 }
		 else{
			 echo $course_name->course_name; ?> (<?php echo $batch->name;
		 }
		?>)
	</div>
