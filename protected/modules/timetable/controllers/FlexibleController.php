...
class FlexibleController extends RController
{
	...
	/**
	 * Specifies the access control rules.nweekdays
	...
	public function beforeAction(){
		...
		if(in_array($action, $redirect_actions)){
			if(Configurations::model()->timetableFormat(isset($_REQUEST['id'])?$_REQUEST['id']:NULL)==1){ // timetable format is not selected as course level					
				$params	= array('/timetable/weekdays/'.$action);
    ...
    public function actionPdf()
    {
        ...
        Yii::app()->osPdf->generate("application.modules.timetable.views.flexible.exportpdf", $batch_name, array(), 0, "", "A4", 5, 5, 5, 5);
	}
	...
	public function actionFullPdf()
	{
		...
		Yii::app()->osPdf->generate("application.modules.timetable.views.flexible.fulltimetablepdf", $batch_name, array(), 0, "", "A4", 5, 5, 5, 5);
	}
