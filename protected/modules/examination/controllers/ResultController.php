	...
	public function actionIndex()
	{
		...
		if($_GET['search_id']=='1' and isset($_GET['search_button']))
		{
			...
			elseif($_GET['search_id']=='1' and $_GET['course']!=0 and $_GET['batch_id']==0 and $_GET['group_id']==0 and $_GET['exam_id']==0)
			{
				...
				$criteria->params		= array(':is_active'=>1,':is_deleted'=>0,':course'=>$_GET['course']);
				$batches  = Batches::model()->findAll($criteria);
				foreach($batches as $batch){
					$batch_id[] = $batch->id;
				}
			...
			{ 
				...
				$criteria_1->condition="`eg`.batch_id=:batch_id AND `eg`.is_published =:is_published";
	        	$criteria_1->params=array(':batch_id'=>$batch,':is_published'=>1);
