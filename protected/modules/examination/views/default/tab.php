...
<li> <?php             
                ...
                else if((Yii::app()->controller->id=='examScores' and Yii::app()->controller->action->id=='create'))
                {                     
                    $exam_id	=	$_GET['examid'];
	...
	<table border="0" cellpadding="0" cellspacing="0p" width="100%">
        ...
        <tr>
        	<th class="course-icon"><?php echo Yii::t('app', 'Course'); ?></th>
                <?php   if(Configurations::model()->isSemesterEnabledForCourse($batch->course_id)){?>
            ...
            <th class="batch-icon"><?php echo Yii::app()->getModule('students')->fieldLabel("Students", "batch_id") ?></th>
			 ...
			 <?php 
            if((Yii::app()->controller->id=='examScores' and Yii::app()->controller->action->id=='update'))
            {                                                                       
                if((isset($_REQUEST['sid']) && $_REQUEST['sid']!=NULL))
                {?>
                    <th class="exam-icon"><?php echo Yii::t('app', 'Subject'); ?></th>
                <?php }                   
            } ?>
            <?php 
            if((Yii::app()->controller->id=='examScores' and Yii::app()->controller->action->id=='create') or (Yii::app()->controller->id=='exams' and Yii::app()->controller->action->id=='create'))
            {                                                                       
                    ...
                    <td>
					 ...
					 <?php 
						...
						{                                                                       
							if((isset($_REQUEST['sid']) && $_REQUEST['sid']!=NULL))
							{
								$subject = Subjects::model()->findByAttributes(array('id'=>$exam_model->subject_id));?>
                    ...
                    if(Yii::app()->controller->id=='exams' and Yii::app()->controller->action->id=='create')
                    {
                        ...
                        <td>
                            <?php 
                                ...
                                $group= ExamGroups::model()->findByPk($exam_group_id);
                                if($group)
                                {
                                    echo $group->name;
                                }                                
    ...
    <thead>
        <tr>
            <td>
                ...
                <?php /*?><table border="0" cellpadding="0" cellspacing="0p" width="30%" class="exam-table-gris">
                    <thead>
                        <tr>
                            <td width="48%">
                            	<a href="#" class="change-btn">Create Exam</a>
                            </td>
                            <td width="50%">
                            	<a href="#" class="change-btn">Grading Levels</a>
                            </td>
                        </tr>   
                    </thead>
                </table><?php */?>
