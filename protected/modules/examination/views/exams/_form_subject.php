...
function new_1(id)
{
	...
	if(!isNaN(val))
	{
		...
		for(i=1;i<=id;i++)
		{
	    	...
	    	document.getElementById('Exams_maximum_marks_'+i).value = val;
		}
	}
	else
	{
...
function old_1(id)
{
	...
	if(!isNaN(val))
	{
		...
		for(i=1;i<=id;i++)
		{
			document.getElementById('Exams_minimum_marks_'+i).value = val;
		}
	}
	else
	{
...
$("form#exams-form").submit(function(e) {
	...
	if (textBox == "") {
		...
		$.ajax({
			url:'<?php echo Yii::app()->createUrl("/examination/exams/create", array("id"=>$_REQUEST['id'], "exam_group_id"=>$_REQUEST['exam_group_id']));?>',
			type:'POST',
			data:data,
			dataType:"json",
