...
$model = Configurations::model()->findByAttributes(array('id'=>39));?>
	...
	if($model->config_value == 1)
        ...
        'items'=>array(
			  ...
			  array('label'=>''.'<h1>'.Yii::t('app','Complaints').'</h1>', 'visible'=>Yii::app()->user->checkAccess('Complaints.Index') or Yii::app()->user->checkAccess('Complaints.Feedbacklist') or Yii::app()->user->checkAccess('Complaints.Create')), 
				 ...
				 array('label'=>''.Yii::t('app','Category').'<span>'.Yii::t('app','Manage Category').'</span>', 'url'=>array('/complaints/categories') ,'linkOptions'=>array('class'=>'lbook_ico'),'visible'=>Yii::app()->user->checkAccess('Admin'),
							...
							array('label'=>''.Yii::t('app','Complaint List').'<span class="count">'.count($unread_msg).'</span><span>'.Yii::t('app','Complaint List').'</span>', 'url'=>array('/complaints/index') ,'linkOptions'=>array('class'=>'mg_ico'),'visible'=>Yii::app()->user->checkAccess('Complaints.Index'),
						...
						array('label'=>''.Yii::t('app','Complaint List').'<span>'.Yii::t('app','Complaint List').'</span>', 'url'=>array('/complaints/feedbacklist') ,'linkOptions'=>array('class'=>'mg_ico'),'visible'=>Yii::app()->user->checkAccess('Complaints.Feedbacklist') && !Yii::app()->user->checkAccess('Admin'),
						...
						array('label'=>''.Yii::t('app','Register Complaint').'<span>'.Yii::t('app','Register Complaint').'</span>', 'url'=>array('/complaints/create') ,'linkOptions'=>array('class'=>'ne_ico'),'visible'=>Yii::app()->user->checkAccess('Complaints.Create') && !Yii::app()->user->checkAccess('Admin'),
