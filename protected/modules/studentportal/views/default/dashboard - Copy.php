...
<div class="main_box">
       ...
       <?php if(Configurations::model()->studentAttendanceMode() != 2){?>     
            ...
            <div class="table-responsive">
                ...
                if(count($batchstudents)>1){
                    $student_batch_id = isset($_REQUEST['bid'])?$_REQUEST['bid']:'';
                    $batch_label    =   Yii::app()->getModule('students')->batchLabel("Students", "batch_id");                    
                    $list   = CHtml::listData($batchstudents, 'id', 'name');
                    echo CHtml::dropDownList('batch_id', $student_batch_id, $list ,array('encode'=>false,'id'=>'batch_drop','class'=>'form-control', 'empty'=>  Yii::t('app', 'Select')." ".$batch_label));
                }
                ?>
       ...
       <?php } ?>
            ...
            <?php 
                ...
                if(count($batchstudents)>1){
                    ...
                    $batch_label    =   Yii::app()->getModule('students')->batchLabel("Students", "batch_id");                    
                    $list   = CHtml::listData($batchstudents, 'id', 'name');
                    echo CHtml::dropDownList('batch_id', $student_batch_id, $list ,array('encode'=>false,'id'=>'batch_drop_exam','class'=>'form-control', 'empty'=>  Yii::t('app', 'Select')." ".$batch_label));
                }
                ?>
  ...
  <?php if($student_batch_id and ($student_batch_id !=NULL or $student_batch_id!=0))
   ...
   ?>
            ...
            <div class="col-md-4">
            <div class="panel-heading">
              <h4 class="panel-title dashbord_icon"><i class="fa fa-columns"></i><?php echo Yii::t('app','Time table');?></h4>
              <?php 
