...
class MessageController extends RController
{
			...
			if($validate)
			{
				...
				if($msg->save()){
					...
					if(Configurations::model()->isAndroidEnabled()){
						$device_arr		= array();
						$receiver		= User::model()->findByAttributes(array('email'=>$conv->to));
						$sender			= User::model()->findByAttributes(array('id'=>$conv->initiator_id));
						$sender_name	= Configurations::model()->getUserName($msg->sender_id);
		...
		if(isset($_POST['text']))
		{
			...
			if($validate)
			{
				...
				if($reply->save()){
					...
					if(Configurations::model()->isAndroidEnabled()){						
						$device_arr		= array();
						$receiver		= User::model()->findByAttributes(array('id'=>$reply->recipient_id));
						$sender			= User::model()->findByAttributes(array('id'=>$reply->sender_id));
						$sender_name	= Configurations::model()->getUserName($reply->sender_id);
						...
						foreach($model as $value){								
							...
							$argument_arr 	= array('message'=>$message, 'content' => html_entity_decode(ucfirst(strip_tags($reply->text))), 'device_id'=>array($value->device_id), 'sender_email'=>$receiver->email, 'sender_name'=>$sender_name, 'conversation_id'=>$conv->conversation_id, 'subject'=>html_entity_decode(ucfirst($conv->subject)));                
							Configurations::model()->devicenotice($argument_arr, $push_notifications[$key]['title'], 'inbox');
						}						
					}
				}
	...
	public function actionView()
	{
		...
		$conv = Mailbox::model()->findAll($criteria);*/
