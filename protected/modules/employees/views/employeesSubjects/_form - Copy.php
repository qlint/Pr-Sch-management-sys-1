...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	if(isset($_REQUEST['bat']))
	{
		...
		$batches = Batches::model()->findByPk($_REQUEST['bat']);
		$data_1 = CHtml::listData(Subjects::model()->findAll(array('join' => 'JOIN batches ON batch_id = batches.id','condition'=>'batches.course_id=:id and batches.is_active=:is_active and batches.is_deleted=:is_deleted and elective_group_id=:x','params'=>array(':id'=>(int) $batches->course_id, ':is_active'=>1, ':is_deleted'=>0,':x'=>0))),'id','batchname');			  
...
?>
    ...
    echo CHtml::dropDownList('bid','',$data2,array('prompt'=>Yii::t('app','Select Course - Batch'),'onchange'=>'coursename()','id'=>'bid','options'=>array($sel=>array('selected'=>true))));
    echo '</div>';
    echo '<div class="two-Inputarea"><span>'.Yii::t('app','Subject').'</span>&nbsp;&nbsp;'; ?>
    ...
    echo CHtml::dropDownList('elective','',$data_2,array('prompt'=>Yii::t('app','Select Subject'),'id'=>'elect','onchange'=>'batchname()','options'=>array($sel_1=>array('selected'=>true)))); 
