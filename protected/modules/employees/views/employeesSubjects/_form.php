...
window.location= 'index.php?r=employees/employeesSubjects/create&b_id='+id;	
}
function coursename()
{
...
function batch()
{
var id_1 = document.getElementById('b_id').value;
var id = document.getElementById('sub').value;
window.location= 'index.php?r=employees/employeesSubjects/create&b_id='+id_1+'&sub='+id;	
}
function batchname()
{
...
window.location= 'index.php?r=employees/employeesSubjects/create&b_id='+id_1+'&sub='+id+'&dept='+dep;		
}
function depname()
{
...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	$model_batch =  Batches::model()->findAll("is_deleted=:x AND academic_yr_id=:y AND is_active=:z", array(':x'=>0,':y'=>$year,':z'=>1),array('order'=>'course_id DESC')) ;
    foreach ($model_batch as $model_b){							
		$data[$model_b->id] = $model_b->course123->course_name.' ( '.$model_b->name.' )';
	}
	...
	echo '<div class="formCon"><div class="formConInner"><div class="two-Inputarea"><span>'.Yii::t('app','Course').' / '.Yii::app()->getModule('students')->fieldLabel("Students", "batch_id").'</span>&nbsp;&nbsp;&nbsp;&nbsp;';
	echo CHtml::dropDownList('id','',$data,array('prompt'=>Yii::t('app','Select Course').' / '.Yii::app()->getModule('students')->fieldLabel("Students", "batch_id"), 'encode'=>false,'onchange'=>'course()','id'=>'b_id','options'=>array($sel=>array('selected'=>true)))); 
	echo '</div>';
	...
	if(isset($_REQUEST['b_id']))
	{
		...
		$batches = Batches::model()->findByPk($_REQUEST['b_id']);
		$data_1 = CHtml::listData(Subjects::model()->findAll(array('condition'=>'batch_id=:id and  elective_group_id=:x','params'=>array(':id'=>(int) $batches->id,':x'=>0))),'id','name');			  
	...
	echo CHtml::dropDownList('sub','',$data_1,array('prompt'=>Yii::t('app','Select Subject'),'id'=>'sub','onchange'=>'batch()','options'=>array($sel_1=>array('selected'=>true)))); 
...
if(isset($_REQUEST['sub']))
{
            ...
            <table border="0" cellpadding="0" cellspacing="0" width="100%">
                ...
                ?>
                    <tr>
						...
						?>
                        	<td>
							<?php echo CHtml::link(Yii::t('app','Remove'), "#", array('submit'=>array('deleterow','id'=>$emp_sub_1->id,'sub'=>$_REQUEST['sub'],'b_id'=>$_REQUEST['b_id'],'cou'=>$_REQUEST['cou'],'dept'=>$_REQUEST['dept']), 'confirm'=>Yii::t('app','Are you sure?'), 'csrf'=>true));?>
                            </td>
						<?php
						}
						?>
	...
	if(($year == $current_academic_yr->config_value) or ($year != $current_academic_yr->config_value and $is_insert->settings_value!=0))
	{
		...
		echo CHtml::dropDownList('dep','',CHtml::listData(EmployeeDepartments::model()->findAll(),'id','name'),array('prompt'=>Yii::t('app','Select'),'id'=>'depart','onchange'=>'departme()','encode'=>false,'options'=>array($sel_2=>array('selected'=>true))));
				...
				<div class="tableinnerlist">
					<table width="100%" cellpadding="0" cellspacing="0">
						...
						foreach($employee as $employee_1)
						{
							...
							echo '<td>'.CHtml::link(Yii::t('app','Assign'), "#", array('submit'=>array('employeesSubjects/Assign','emp_id'=>$employee_1['id'],'sub'=>$_REQUEST['sub'],'b_id'=>$_REQUEST['b_id'],'cou'=>$_REQUEST['cou'],'dept'=>$_REQUEST['dept']), 'confirm'=>Yii::t('app','Are you sure?'), 'csrf'=>true)).'</td>';
							echo '</tr>';
						}
						?>
					</table>
...
?>
	...
	$models =  Batches::model()->findAll("is_deleted=:x AND academic_yr_id=:y AND is_active=:z", array(':x'=>0,':y'=>$year,':z'=>1),array('order'=>'course_id DESC')) ;
    foreach ($models as $model_1){							
		$data2[$model_1->id] = $model_1->course123->course_name.' ( '.$model_1->name.' ) ';
	}
    ...
    echo '<div class="formCon"><div class="formConInner"><div class="two-Inputarea"><span>'.Yii::t('app','Course').' / '.Yii::app()->getModule('students')->fieldLabel("Students", "batch_id").'</span>&nbsp;&nbsp;&nbsp;&nbsp;';
    echo CHtml::dropDownList('bid','',$data2,array('prompt'=>Yii::t('app','Select Course').' / '.Yii::app()->getModule('students')->fieldLabel("Students", "batch_id"),'encode'=>false,'onchange'=>'coursename()','id'=>'bid','options'=>array($sel=>array('selected'=>true))));
    echo '</div>';
    echo '<div class="two-Inputarea"><span>'.Yii::t('app','Elective / Elective Group').'</span>&nbsp;&nbsp;'; ?>
	...
	//	$data_2 = CHtml::listData(Electives::model()->findAll(array('join' => 'JOIN elective_groups ON elective_group_id = elective_groups.id','condition'=>'elective_groups.batch_id=:id','params'=>array(':id'=>(int) $_REQUEST['bid']))),'id','name');
		$elect_datas =  Electives::model()->findAll(array('join' => 'JOIN elective_groups ON elective_group_id = elective_groups.id','condition'=>'elective_groups.batch_id=:id','params'=>array(':id'=>(int) $_REQUEST['bid'])));	
			
			foreach ($elect_datas as $elect_d){							
				$data_2[$elect_d->id] = $elect_d->name.' ( '. $elect_d->electiveg->name.' ) ';
			}
    ...
    echo CHtml::dropDownList('elective','',$data_2,array('prompt'=>Yii::t('app','Select Elective / Elective Group'),'id'=>'elect','onchange'=>'batchname()','options'=>array($sel_1=>array('selected'=>true)))); 
