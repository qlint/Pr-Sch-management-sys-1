...
class EmployeesController extends RController
{
		...
		{	
			...
			*/
			if (($list['experience_year']&&$list['experience_year']!='0')||($list['experience_month']&&$list['experience_month']!='0')) {
				...
				if(isset($list['date_of_birth']) and $list['date_of_birth']!=NULL){
				...
				if($model->save())
				{
						//adding user for current employee
						$user=new User;
						$profile=new Profile;
		...
		if(isset($_POST['Employees']))
		{
			...
			$model =$model->findByAttributes(array('id'=>$_REQUEST['id']));
				...
				if($model->save()){
						...
						ActivityFeed::model()->saveFeed(Yii::app()->user->Id,'23',$model->id,Employees::model()->getTeachername($model->id),NULL,NULL,NULL); 
			...
			if($model->save())
			{
				...
				foreach($results as $key => $value)
				{
					if($key != 'updated_at')
					{
						...
						//Adding activity to feed via saveFeed($initiator_id,$activity_type,$goal_id,$goal_name,$field_name,$initial_field_value,$new_field_value)
						ActivityFeed::model()->saveFeed(Yii::app()->user->Id,'24',$model->id,Employees::model()->getTeachername($model->id),$model->getAttributeLabel($key),$old_model[$key],$value); 
	 ...
	 public function actionDeletes()
	{
		if(Yii::app()->request->isPostRequest){		
			...
			//Adding activity to feed via saveFeed($initiator_id,$activity_type,$goal_id,$goal_name,$field_name,$initial_field_value,$new_field_value)
			ActivityFeed::model()->saveFeed(Yii::app()->user->Id,'25',$model->id,Employees::model()->getTeachername($model->id),NULL,NULL,NULL);
			echo $val;
