...
class Batches extends CActiveRecord
{
        ...
        public function getCourse()
	{
        ...
        public static function getAssesmentBatches()
        {            
            $year       =   Yii::app()->user->getState('ses_session_yr');
            $semester   =   Yii::app()->user->getState('ses_semester_id');
            $batch_id   =   Yii::app()->user->getState('ses_batch_id');
            $batch_data= array();   
            
            if(isset($semester) && $semester!=NULL)
            { 
                if($semester== 0)
                { 
                        $criteria=new CDbCriteria;
                    ...
                    $data = Batches::model()->findAll('academic_yr_id=:x AND is_deleted=:y AND is_active=1 AND semester_id=:sem_id',array(':x'=>$year,':y'=>0,':sem_id'=>$semester));	
                    foreach ($data as $da){							
            ...
            else if(isset($semester) && $semester=='')
            {                  
                $data = Batches::model()->findAll('academic_yr_id=:x AND is_deleted=:y AND is_active=1',array(':x'=>$year,':y'=>0));
                foreach ($data as $da){							
                    $batch_data[$da->id] = $da->name.' ( '.$da->course123->course_name.' )';
                }
                return $batch_data;              
