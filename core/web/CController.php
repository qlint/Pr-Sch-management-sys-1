...
class CController extends CBaseController
{
	...
	{
		$c_controller	= strtolower(Yii::app()->controller->id);	// current controller
		$c_action		= strtolower(Yii::app()->controller->action->id);	// current action
		
		if(!Yii::app()->user->isGuest && $c_controller!="androidapi" && $c_controller!="site" && $c_action!="offline"){
			//check any offline setup enabled
			$offline_status		= SystemOfflineSettings::model()->updateStatus();
			if($offline_status==1){
				//check user is allowed in offline period
				$user 				= User::model()->findByPk(Yii::app()->user->id);
				$user_login_status	= SystemOfflineSettings::model()->checkUpdate($user->username);
				if($user_login_status!=1){
					Yii::app()->user->logout();
					//redirect to offline message page
					$this->redirect(array('/site/offline'));
				}
			}
		}
		return true;
